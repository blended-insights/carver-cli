    ...
    .action(async (options) => {
      try {
        ...
        if (options.apiKey) {
          ...
          if (method === 'API Key') {
            ...
            const { pkeyPath } = await prompt<PromptPkeyPathResponse>({
              ...
              initial: path.join(process.env.HOME || process.env.USERPROFILE || '', '.ssh', 'id_rsa'),
            });
        ...
        if (success) {
          ...
          if (!isConnected) {
            logger.warn('Authenticated but unable to connect to API. Check your network connection.');
          } else {
            // Check version compatibility
            const compatibility = await apiService.checkVersionCompatibility();
            if (!compatibility.compatible) {
              logger.warn(`Your CLI version may not be fully compatible with the API. Recommended version range: ${compatibility.minVersion} - ${compatibility.maxVersion}`);
            }
              ...
              if (projects.length > 0) {
                ...
                if (setDefault) {
                  ...
                  if (projectIndex < 1 || projectIndex > projects.length) {
                    logger.error(`Invalid project number. Must be between 1 and ${projects.length}`);
                  } else {
                ...
                logger.info('No projects found. You can create a new project with "carver create-project"');
              }
            } catch (error) {
