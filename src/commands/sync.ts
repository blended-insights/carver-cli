  ...
  program
    ...
    .action(async (options) => {
      try {
        ...
        if (!projectId) {
          logger.error('Project ID required. Please specify with --project or set a default project.');
          process.exit(1);
        }
        ...
        if (options.watch) {
          ...
          if (queueCounts.pending > 0 || queueCounts.failed > 0) {
            logger.info(`Processing ${queueCounts.pending + queueCounts.failed} queued operations...`);
          ...
          logger.info(`Synchronization complete. ${syncResult.added || 0} added, ${syncResult.updated || 0} updated, ${syncResult.deleted || 0} deleted.`);
        }
      } catch (error) {
